...
internal class LoadingResource
{
    ...
    public bool CacheResult; // true to store into a cache after loading
...
public class GroupLoader
{
    ...
    */
    public async void LoadResources(List<Resource> resources, Action finishCallback = null)
    {
        ...
        foreach (var resource in resources)
            LoadResource(resource.Name, resource.FinishCallback, resource.CacheResult, resource.InstantiateResult,
                resource.Transform);
        while (!IsAllDone())
        {
            await Task.Delay(75);
    ...
    */
    private async void LoadResource(string name, Action<object> finishCallback = null, bool cacheResult = false,
        bool instantiateResult = false, Transform transform = null)
    {
        if (HasResource(name))
        {
        ...
        _resources.Add(new LoadingResource
        {
            Name = name, FinishCallback = finishCallback, CacheResult = cacheResult,
        ...
        resourceRequest.completed += o =>
        {
            ...
            {
                var go = transform == null
                    ? UnityEngine.GameObject.Instantiate(resourceRequest.asset)
                    : UnityEngine.GameObject.Instantiate(resourceRequest.asset, transform);
                if (finishCallback != null) finishCallback.Invoke(go);
            }
            else
            {
                if (finishCallback != null)
                    finishCallback.Invoke(resourceRequest.asset); // invoke finish callback for this resource
            }

            _resources[_resources.IndexOf(_resources.Find(lr => lr.Name == name))].Result =
                resourceRequest.asset; // set this assets result obj
            if (!_resources.Find(lr => lr.Name == name).CacheResult)
                _resources.Remove(_resources.Find(lr => lr.Name == name)); // delete non cached items
            _progress += 1 / _numResources;
        };
    }
